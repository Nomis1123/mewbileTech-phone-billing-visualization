...
def new_month(customer_list: list[Customer], month: int, year: int) -> None:
    ...
    """
    # Finds all customers in the customer list
    # and advances them by 1 month
...
def process_event_history(log: dict[str, list[dict]],
    ...
    # finds the type of event within the log list
    for event_data in log['events']:
        if event_data['type'] == 'sms':  # ignoring the sms data type
            continue
        ...
        # assigns all the data found in even data
        srcloc = (event_data['src_loc'][0], event_data['src_loc'][1])

        dstloc = (event_data['dst_loc'][0], event_data['dst_loc'][1])

        call = Call(event_data['src_number'], event_data['dst_number'],
                    billing_date, event_data['duration'], srcloc, dstloc)

        cstr = find_customer_by_number(event_data['src_number'], customer_list)

        cstr.make_call(call)

        cstr = find_customer_by_number(event_data['dst_number'], customer_list)

        cstr.receive_call(call)
